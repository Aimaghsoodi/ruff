...
impl<'db> Constraint<'db> {
    ...
    fn normalized(&self, db: &'db dyn Db) -> Self {
        Self {
            ...
            lower: self.lower.normalized(db),
            upper: self.upper.normalized(db),
        }
    }
}
...
impl ConstraintId {
    ...
    fn intersect<'db>(
        ...
        let lower = UnionType::from_elements(db, [self_constraint.lower, other_constraint.lower]);
        let upper =
        ...
        while let Some((left_constraint, right_constraint)) = to_visit.pop() {
            ...
            match left_constraint.intersect(db, builder, right_constraint) {
                IntersectionResult::Simplified(intersection_constraint_data) => {
                    let intersection_constraint_data = intersection_constraint_data.normalized(db);
